...
FILE * output_file = fopen ("dump.txt", "wb");
...
Elem_t * create_data_canary (Stack_t * stack)
{
    ...
    stack_data++;
    stack_data [stack->capacity * sizeof (Elem_t) / sizeof (int) + 
               (stack->capacity * sizeof (Elem_t) % sizeof (int) != 0)] = RIGHT_DATA_CANARY;
...
Elem_t * recreate_data_canary (Stack_t * stack, size_t new_capacity)
{
    assert (stack);
    
    int * stack_data = (int *) stack->data;

    int temp_canary = stack_data[stack->capacity*sizeof (Elem_t)/sizeof (int)];

    stack_data--;

    stack_data = (int*) recalloc (stack_data, sizeof (LEFT_DATA_CANARY) + new_capacity*sizeof (Elem_t) + sizeof (RIGHT_DATA_CANARY), 1);

    stack_data++;

    stack_data[new_capacity*sizeof (Elem_t)/sizeof (int)] = temp_canary;
...
void calculate_stack_hash (Stack_t * stack)
{
    ...
    stack->hash += calculate_hash (&stack, sizeof (stack));
